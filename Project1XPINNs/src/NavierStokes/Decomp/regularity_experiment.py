...
v_hess_psi = vmap(hess_psi, (None, 0))
...
def interface_loss_factory(i: int, j: int, weight = 1) -> LFunc:
    ...
    def interface_loss(params: Params, args: dict[str, Array]) -> Array:
        ...
        ### Hessian interface
        hess_psi_j = v_hess_psi(params_j, inter_points)
        hess_psi_i = v_hess_psi(params, inter_points)

        hess_loss = 4 * np.mean(np.square((hess_psi_j - hess_psi_i)))

        return weight * (res_ij + avg_ij + neumann + hess_loss)
...
p1.interface_loss = interface_loss_factory(1, 0, weight = 20 * w_interface)
...
our_model_path = model_path / "NavierStokes"/ "Decomp_2"/ f"{file_train[:-5]}_{w_boundary}_{w_interface}_{w_residual}_iterations={n_iter}_Hessian_rightfocus"
