    ...
    def generate_data(self, num_points: List[Tuple[int, int]]):
        for i, region in enumerate(self.regions):
            ...
            if isinstance(region, ConvexPolygon):
                self.all_points = self._generate_data_ConvexPoly(
                    region, region_num_points, i, other_regions, self.all_points)
            else:
                self.all_points = self._generate_data_Circle(
                    region, region_num_points, i, other_regions, self.all_points)

    @staticmethod
    def _generate_data_ConvexPoly(region: ConvexPolygon, region_num_points: Tuple[int, int], i, other_regions: List[Shape], all_points):
        ...
        points_per_edge = np.round(
            (edge_lengths / total_perimeter) * n_boundary).astype(int)
        ...
        all_points[region_key] = {
            'interior': interior_points, 'boundary': np.array(boundary_points)}

        return all_points

    @staticmethod
    def _generate_data_Circle(region: ConvexPolygon, region_num_points: Tuple[int, int], i, other_regions: List[Shape], all_points):
        region_key = f'region{i}'
        n_interior, n_boundary = region_num_points
...
def _generate_interior_points(x_min, x_max, t_min, t_max, n_interior: int, region: Shape, other_regions: List[Shape]):
