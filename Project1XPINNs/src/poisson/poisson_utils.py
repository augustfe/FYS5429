...
def boundary_loss_factory(pinn_model:PINN,target: float | Array, weight: float =1) -> LFunc:
    """
...
def interior_loss_factory(pinn_model: PINN, rhs: Callable[[Array], Array] | Array, weight : int = 1) -> LFunc:
    """ 
...
def interface_loss_factory(xpinn_model:XPINN,i: int, j: int, weight : float = 1) -> LFunc:
    a, b = sorted([i, j])
