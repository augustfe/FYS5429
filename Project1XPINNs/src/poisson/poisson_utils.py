...
LossFunction = Callable[[Params, dict[str, Array]], Array]
...
def boundary_loss_factory(
    ...
    Args:
        ...
        target (float | Array): A singular value or an array of values corresponding to boundary points
        weight (float, optional): The weight of the boundary loss. Defaults to 1.0.
    ...
    def boundary_loss(params: Params, points: dict[str, Array]) -> Array:
        """Calculate the boundary loss for the Poisson problem

        Args:
            params (Params): The parameters of the network
            points (dict[str, Array]): The points of the domain

        Returns:
            Array: The boundary loss
        """
        pts = points["boundary"]
...
def interior_loss_factory(
    ...
    Args:
        ...
        weight (float, optional): The weight of the interior loss. Defaults to 1.0.
    ...
    def hess(params: Params) -> Callable[[Array], Array]:
        """Compute the hessian of the model at a given point

        Args:
            params (Params): The parameters of the network

        Returns:
            Callable[[Array], Array]: The hessian of the model at a given point
        """
        return hessian(lambda x: pinn_model.model(params, x))

    # If the rhs is a function, we want to compute the residual at the point
    if callable(rhs):

        def residual(params: Params, x: Array) -> Array:
    ...
    def interior_loss(params: Params, points: dict[str, Array]) -> Array:
        """Interior loss for the Poisson problem
...
def interface_loss_factory(
    ...
    """Interface loss factory for the Poisson problem

    Args:
        xpinn_model (XPINN): Your XPINN model
        i (int): Index of the first PINN
        j (int): Index of the second PINN
        weight (float, optional): The weight of the interface loss. Defaults to 1.0.

    Returns:
        LFunc: The interface loss function between index i and j
    """
    a, b = sorted([i, j])
    pi = xpinn_model.PINNs[i]

    def interface_loss(params: Params, args: dict[str, Array]) -> Array:
        """Interface loss for the Poisson problem between two PINNs

        Args:
            params (Params): The parameters of the network
            args (dict[str, Array]): The points of the domain

        Returns:
            Array: The interface loss
        """
...
def create_PINN_domain(internal_points: int, boundary_points: int) -> None:
    """Generate a domain for the Poisson problem with one PINN.

    Args:
        internal_points (int): Number of internal points
...
def create_PINN_test_domain(n: int = 1000) -> None:
    """Generate a testing domain for the Poisson problem with one PINN.

    Args:
        n (int, optional): Number of testing points. Defaults to 1000.
    """
    vertecies = np.array([[0, 0], [0, 1], [1, 1], [1, 0]]).reshape(4, -1)
    square = ConvexPolygon(
        vertecies,
...
def crude_rel_L2(prediction: Array, true_value: Array, true_integral: float) -> Array:
    """Estimation of the relative L2 error over the domain

    Args:
        prediction (Array): Predicted values
        true_value (Array): True values
        true_integral (float): True integral
    """
...
def create_XPINN_domain(
    ...
    """Generate a domain for the Poisson problem for XPINNs.
...
def create_XPINN_test_domain(n: int = 1000) -> None:
    """Generate a testing domain for the Poisson problem for XPINNs.
